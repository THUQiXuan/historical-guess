...
问古是一个可在本机运行的中文历史人物猜谜 Web 应用：后台模型选人、回答是否问题、判断姓名猜测，还可自由讨论范围与复盘；浏览器负责游戏界面；SQLite 保存游戏与聊天。前端无需构建，也不加载外部字体、脚本或图片。
...
默认使用 `gpt-5.6-sol`，普通提问与自由聊天使用 `low` 推理强度，选人和身份猜测使用 `medium`，并请求 `fast`。需要变更时，复制配置模板并编辑本地文件：
...
| `CODEX_EFFORT` | `low`，普通是否问题的推理强度 |
| `CODEX_SELECT_EFFORT` | `medium`，人物范围筛选与选人的推理强度 |
| `CODEX_GUESS_EFFORT` | `medium`，姓名猜测与身份核对的推理强度 |
| `CODEX_CHAT_EFFORT` | `low`，自由聊天的推理强度 |
| `CODEX_SERVICE_TIER` | `fast`；不支持时可设为 `default` |
| `CODEX_AGENT_TIMEOUT` | `180`，一次裁判请求的超时秒数 |

Fast 的可用性和额度消耗取决于模型与账户，参见 [Codex Speed](https://learn.chatgpt.com/docs/agent-configuration/speed)。本项目开发环境中，`fast` 在 App Server 返回的有效配置中显示为 `priority`。一次小样本实测约为：后台启动 13.5 秒、选人 6.0 秒、提问 4.9 秒；这些不是性能保证，首次安装依赖和复杂范围筛选可能更久。
...
| `OPENAI_REASONING_EFFORT` | 可选，普通提问及未设置专用值时的 `reasoning_effort` |
| `OPENAI_SELECT_EFFORT` | 可选，选人请求的 `reasoning_effort` |
| `OPENAI_GUESS_EFFORT` | 可选，姓名猜测与身份核对的 `reasoning_effort` |
| `OPENAI_CHAT_EFFORT` | 可选，自由聊天的 `reasoning_effort` |
| `OPENAI_SERVICE_TIER` | 可选，服务商支持的 `service_tier` |
| `OPENAI_TIMEOUT` | 请求超时秒数，默认 `120` |

未设置专用推理变量时，会沿用 `OPENAI_REASONING_EFFORT`；它也为空时，不发送 `reasoning_effort` 字段。专用变量显式留空也可省略该类请求的推理字段。不支持推理强度或服务等级字段的服务，请将对应值留空。修改 `.env` 后重启。`.env` 已被 Git 忽略；只在自己的本地文件中填写密钥。
...
判定口径：明确投降或归附计入“曾属于某阵营”；“任职或效力于某朝廷”须有实际服务证据。猜测不仅要是一个合法人物姓名，还必须指向本局选定的同一个人。若完整姓名或别名在题库中对应其他人物 ID，服务器会否决模型的误判正确。

若维护者依据史料复核，界面会标明更正或撤回，原判保留在数据库；撤回的问题退还次数，不再作为后续推理的有效线索。历史上误判为猜中的记录也可复核并保留原判。由于答案已经揭晓，这类游戏会保持结束状态，不再恢复为可玩的进行中游戏。

## 自由聊天与复盘

游戏下方的「史边闲谈」提供独立聊天窗口，回复可以是完整说明，不受游戏里的「是 / 否」限制。

- **讨论范围**：开局前讨论想猜的人物、难度或史书范围，也可随时筹划下一局。收到范围建议后，点击「采用此范围」会填入开局设定；不会自动开局，也不改变当前游戏。
- **本局交流**：开局后自动关联当前游戏，可讨论玩法与规则。后台不会向进行中游戏的聊天模型提供秘密人物、本局问答记录或内部裁判依据；获取本局人物线索仍须使用上方的提问与猜测区域。
- **本局复盘**：游戏结束后可自由讨论已揭晓人物和全部问答。点击记录旁的「解释这题」，会将该题填入聊天输入框；点击「发送」后即可追问依据。复盘可以指出疑点，不会自行修改游戏记录。

聊天**不限轮数，不占用提问或猜测次数**，每条最多 **10,000 字符**。`Enter` 发送，`Shift + Enter` 换行。等待聊天回复时仍可操作游戏。

范围讨论与每局聊天分别保存；切换或恢复游戏会载入对应聊天。全部聊天历史保存在 SQLite，刷新后仍可查看。为控制请求长度，模型每次仅回看最近至多 **48,000 字符的既往聊天**，更早记录仍完整保存在数据库和界面中；涉及较早细节时可以重新贴出相关内容。

## 保存、连接与隐私

游戏与聊天保存在 `var/games.sqlite3`，包括人物快照、规则、问答、猜测、自由聊天和时间信息。刷新或重启服务后可继续；浏览器 Cookie 标识各自的历史，清除 Cookie 或更换浏览器不会自动带回原记录。备份数据库时可先停止服务再复制 `var/`。
